...
Daily aggregator of Michigan-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "MI".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, MI` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Michigan"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/michigan/`
...
```html
<iframe
  src="https://taildraggers.github.io/michigan/"
  title="Taildragger Ads in Michigan"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_MICHIGAN_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
