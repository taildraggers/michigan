"""Aggregates Michigan-located listings from every taildraggers.com
...
ones whose location ends in ", MI".
...
Indiana/Iowa/Kansas/Kentucky/Louisiana/Maine/Maryland/Massachusetts repos'
approach, with the state filter swapped to "MI".
"""
...
PAGE_TITLE = "Taildragger Ads in Michigan"
STATE_ABBR = "MI"
...
_MICHIGAN_LOCATION_RE = re.compile(r",\s*MI\s*$", re.IGNORECASE)


def _is_michigan(location: str) -> bool:
    return bool(_MICHIGAN_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        michigan_rows = [row for row in rows if _is_michigan(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(michigan_rows)} in Michigan")
        all_listings.extend(michigan_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Michigan listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Michigan listings: {len(listings)}")
